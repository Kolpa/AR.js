import {PlaneGeometry, ShaderMaterial, Mesh, Vector2, Vector3, MeshNormalMaterial, DoubleSide} from 'three';
/**
 * - videoTexture
 * - cloakWidth
 * - cloakHeight
...
export class ArMarkerCloak {
  constructor(videoTexture) {
    const updateInShaderEnabled = true;
    // build cloakMesh
    // TODO if webgl2 use repeat warp, and not multi segment, this will reduce the geometry to draw
    {
      const geometry = new PlaneGeometry(1.3 + 0.25, 1.85 + 0.25, 1, 8).translate(0, -0.3, 0);
      ...
      const cloakMesh = new Mesh(geometry, material);
    ...
    const originalsFaceVertexUvs = [[]];
    // build originalsFaceVertexUvs array
    for (let faceIndex = 0; faceIndex < cloakMesh.geometry.faces.length; faceIndex++) {
      originalsFaceVertexUvs[0][faceIndex] = [];
      originalsFaceVertexUvs[0][faceIndex][0] = new Vector2();
      originalsFaceVertexUvs[0][faceIndex][1] = new Vector2();
      originalsFaceVertexUvs[0][faceIndex][2] = new Vector2();
    }
    // set values in originalsFaceVertexUvs
    for (let i = 0; i < cloakMesh.geometry.parameters.heightSegments / 2; i++) {
    ...
    const originalOrthoVertices = [];
    originalOrthoVertices.push(new Vector3(xMin, yMax, 0));
    originalOrthoVertices.push(new Vector3(xMax, yMax, 0));
    originalOrthoVertices.push(new Vector3(xMin, yMin, 0));
    originalOrthoVertices.push(new Vector3(xMax, yMin, 0));
    {
    // build debugMesh
      ...
      const orthoMesh = new Mesh(geometry, material);
    ...
    function updateUvs(modelViewMatrix, cameraProjectionMatrix) {
      const transformedUv = new Vector3();
    ...
    function updateOrtho(modelViewMatrix, cameraProjectionMatrix) {
      ...
      const transformedUvs = [];
      originalOrthoVertices.forEach(function(originalOrthoVertices, index) {
        const transformedUv = originalOrthoVertices.clone();
        // apply modelViewMatrix and projectionMatrix
        transformedUv.applyMatrix4(modelViewMatrix);
        transformedUv.applyMatrix4(cameraProjectionMatrix);
        // apply perspective
      ...
      for (let i = 0; i < transformedUvs.length; i++) {
